...
一个在本机运行的问卷数据分析工具。上传 CSV/XLSX、定义量表题项和变量角色后，系统会执行可复现的经管论文常用分析，并生成 HTML 报告、Excel 结果表、调节图、JSON 配置与完整审计包。
...
2. 为每个构念添加量表，勾选其题项和反向题，并设置量表上下限。
3. 指定自变量 X、因变量 Y、中介 M、调节 W 和数值型控制变量。
4. 选择分析模块、被调节路径、Bootstrap 次数、置信区间和相关方法。
5. 先查看预检提示，再运行并下载报告或完整审计包。

界面中的“载入示例”会使用 `examples/demo_survey.csv` 填好一套 Model 7 配置，可用于验证全流程。

## 输入约定

- 第一行必须是唯一且非空的列名；每行是一名独立受访者。
- 题项、X/Y/M/W 和控制变量必须能转换为数值。
- 反向计分采用 `最小值 + 最大值 - 原值`。
